...
/* 15.按各科成绩进行排序，并显示排名， Score 重复时保留名次空缺
 ...
 问：重复时保留名次空缺，什么意思？
 答：这是 【按高分的人数】来排位的        -- 若【直接按分数排位】则需要对分数去重 distinct
 如下举例：若重复的名次占了n个人，则空缺 原本的 n-1人的名次

 假设一个分数表中 A-90，B-90，C-90，D-88，E-70
 A、B、C三者并列第一（占了3个位次），所以 第2、3名就不显示了(空缺)，D就是第4名


 A-90 --第1名
 B-90 --第1名
 C-90 --第1名    //第1名占了 【3个位次】，所以 原本的 第2、第3名就空缺了
                //所以下一位就从 第4位次开始排序！！
...
▲ 按 高分人数 来排名：统计出 比我高分的【人】有几个，所得排位结果+1 就是【断层式的排名】
这些高分者中可能出现同分的，也加入“比我高分的人”中，这就 (间接地) 形成了 重复名次空缺现象 
(因为高分的人数没有去重，重复统计的，就会“顶掉了”其中一些名次，造成有些名次会“空缺”)

▲ 按 去重distinct后的分数 来排名：统计出 比我这个分数还高的【分数】有几个，所得排位结果+1 就是【连续的排名】 --重复名次不空缺
(因为分数去重了，所以所有的名次都能显示出来)
...
/* 计算: 比我高分的有几人? 
 ...
 对 高分人数 不去重，得到的就是 【断层式的排名】

 若 对分数去重，按分数排名，得到的就是 【连续的排名】
  count(distinct(biao02.score))+1 as rank
*/
select biao01.*,  count(distinct(biao02.score))+1 as rank
from
   (SC as biao01)
   left join
   (SC as biao02)
